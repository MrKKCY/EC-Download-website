...
<body>
    ...
    foreach ($contents as $item) {
        if ($item!= '.' && $item!= '..') {
            ...
            if (is_file($fullPath)) {
                ...
                if ($fileExtension == 'html' || $fileExtension == 'txt' || $fileExtension == 'png') {
     ...
     * 检查给定的路径是否允许访问。
     *
     * 此函数用于验证用户请求的路径是否在允许的基础路径内，并且不包含可能导致目录遍历攻击的 '..' 序列。
     *
     * @param string $path 要检查的路径。
     * @return bool 如果路径允许访问，则返回 true；否则返回 false。
     */
    function isPathAllowed($path)  
    {
        ...
        foreach ($pathSegments as $segment) {
            // 检查路径段是否为 '..'，如果是则返回 false
            if ($segment == '..') {
                return false;
            }
        }
        // 如果路径通过了所有检查，则返回 true
        return true;
    }
